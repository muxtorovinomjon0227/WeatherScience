...
class AllTemperatures extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        CommText(text: "Today",fontSize: 20.sp,fontWeight: FontWeight.bold).paddingOnly(l: 16.sp),
        ...
        SingleChildScrollView(
          ...
          child: Row(
            children: [
              ...
              _card(center: "${dayModel.main?.feels_like?.truncate() ?? 0} °C", bottom: 'Like'),
              SizedBox(width: 8.sp),
              _card(center: "${dayModel.main?.temp_min?.truncate() ?? 0} °C", bottom: 'Min'),
              SizedBox(width: 8.sp),
              _card(center: "${dayModel.main?.temp_max?.truncate() ?? 0} °C", bottom: 'Max'),
              SizedBox(width: 16.sp),
            ],
          ),
        ),
